...
function findGroups(nodeList) {
  const groups = [];
  nodeList.forEach(node => {
    const connectionIds = node.listAllConnectionIds(nodeList).join();
    //console.log(`node ${node.id}, connections ${util.inspect(connectionIds)}`);
    if (!groups.includes(connectionIds)) {
      groups.push(connectionIds);
    }
  })
  return groups;
}
...
6 <-> 4, 5`;
  ...
  test('solve', assert => {
    ...
    assert.equal(a, 2, 'solution to example should be 2');
    assert.end();
  });

  test('findGroups', assert => {
    const nodeList = parseInput(example);
    const groups = findGroups(nodeList);
    assert.equal(groups.length, 2, 'findGroups should return 2');
    assert.end();
  });
